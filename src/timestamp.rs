...
pub fn convert_iso_string_to_timestamp(iso_string: &String) -> Timestamp {
    let (year, month, day, hour, minute, second, decimal) =
        convert_date_time_string_to_ymdhms(iso_string);
...
fn convert_ymdhms_to_timestamp(
    year: u64,
    month: u64,
    day: u64,
    hour: u64,
...
fn add_year_to_second(ts: u64, year: u64) -> u64 {
    ...
    if year < 1972 {
        ...
        if year >= 2000 {
            ...
            (year_counter, ts) =
                increment_year_and_timestamp(year, year_counter, ts, 400, SECONDS_IN_400_YEARS);
            (year_counter, ts) = increment_leap_year(year, year_counter, ts);
            (year_counter, ts) =
                increment_year_and_timestamp(year, year_counter, ts, 100, SECONDS_IN_100_YEARS);
        }

        (year_counter, ts) = increment_year_and_timestamp(
            year,
            year_counter,
            ts,
            4,
            SECONDS_IN_FOUR_YEARS_WITH_LEAP_YEAR,
        );
        (year_counter, ts) = increment_leap_year(year, year_counter, ts);
...
fn increment_year_and_timestamp(
    year: u64,
    year_counter: u64,
    ts: u64,
    divisor: u64,
...
fn add_month_seconds(year: u64, month: u64) -> u64 {
    ...
    if is_leap_year(year) {
        month_seconds_map = vec![
            0, 2678400, 5184000, 7862400, 10454400, 13132800, 15724800, 18403200, 21081600,
            23673600, 26352000, 28944000, 31622400,
        ];
    } else {
        month_seconds_map = vec![
            0, 2678400, 5097600, 7776000, 10368000, 13046400, 15638400, 18316800, 20995200,
